...
class DataTransformer():
    ...
    def __init__(self, column_target, target_mean, target_std):
...
def clean_data(df, mol2seq, column_smiles, column_target):
    ...
    log('molecules with known atom types=', len(df[mask_known]))
    mask_notna = df[column_target].notna().to_numpy()
    log('molecules with given target value for ', column_target, '=', len(df[mask_notna]))
    mask = np.logical_and(mask_known, mask_notna)
    df_cleaned = df[mask].copy()
    log('molecules with both=', len(df_cleaned))
...
def get_dataloaders_with_calculated_normalized_data(df, column_smiles, column_target, args, train_size, test_size):
    ...
    log('target mean=', mean, 'target std=', std)
    ...
    log('train=', len(x_train), ', val=', len(x_val), ', test=', len(x_test))
