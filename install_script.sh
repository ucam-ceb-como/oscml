...
function check_conda {
	...
	if [ $? -eq 0 ]; then
		...
		echo "Couldnt find conda installation. On Windows, you must run this script from Anaconda Prompt for conda to be correctly located. Aborting installation."
		exit -1
	fi
}
...
function get_data_from_server {
	...
	then
		...
		mkdir -p $DATA_LOCAL
		echo -n "provide your vienna username: "
		read USERNM
...
then
	...
	echo "-e      : re-creates conda environment for this project"
    echo "          (it will remove the current '"$CONDA_ENV_NAME"'"
    echo "          environment, if exists, and create it again"
    echo "          installing all necessary packages)"
	echo "-i      : installs the oscml project as a python package"
    echo "-d      : downloads project data from Vienna"
fi
sleep 5 
